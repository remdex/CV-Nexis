...
export default class extends window.Controller {
    ...
    initDropZone() {
        ...
        const aiButtonTemplate = this.element.querySelector('#' + this.data.get('id') + '-ai-button').innerHTML.trim();
        ...
        this.dropZone = new Dropzone(this.element.querySelector('#' + this.data.get('id')), {
            ...
            init: function () {
                ...
                this.on('addedfile', (e) => {
                    ...
                    const aiButton = Dropzone.createElement(aiButtonTemplate);
                    ...
                    // Configure AI button to manually call the AI controller
                    aiButton.addEventListener('click', (event) => {
                        event.preventDefault();
                        event.stopPropagation();
                        
                        if (!e.data || !e.data.id) {
                            controller.alert('Warning', 'Please wait for file upload to complete before AI processing', 'warning');
                            return;
                        }
                        
                        // Get the AI controller from the same element
                        if (window.application) {
                            const aiController = window.application.getControllerForElementAndIdentifier(controller.element, 'ai');
                            if (aiController) {
                                // Create event object with params property
                                const eventObj = {
                                    preventDefault: () => {},
                                    stopPropagation: () => {},
                                    currentTarget: aiButton,
                                    params: { attachmentId: e.data.id }
                                };
                                aiController.process(eventObj);
                            } else {
                                controller.alert('Error', 'AI controller not found', 'error');
                            }
                        } else {
                            controller.alert('Error', 'AI processing not available', 'error');
                        }
                    });
                    ...
                    e.previewElement.appendChild(aiButton);
                });
                ...
                this.on('removedfile', file => {
                    if (file.hasOwnProperty('data') && file.data.hasOwnProperty('id')) {
                        ...
                        let removeItemAi = document.querySelector(`.files-ai-${file.data.id}`);
                        if(removeItemAi!==null && removeItemAi.parentNode!==null){
                            removeItemAi.parentNode.removeChild(removeItemAi);
                        }
